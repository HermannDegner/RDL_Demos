...
### E（予測誤差）— H は「否定回数」ではない

同リポジトリの [`demos/relational-ecology-lab`](../demos/relational-ecology-lab/)（RDL Living Field）が、
個体に `decision = { prediction, actual }` を持たせて予測誤差を出している。
その**決定オブジェクトのパターンだけ**を移植してある。
...
| 次元 | 予測 | 実測 |
|---|---|---|
| `match` | 次の入力を捉えられるか（実測のEMA） | exact 1.0 / partial 0.6 / miss 0.0 |
| `acceptance` | 応答が受け入れられるか（ノードの confidence） | `y` 1.0 / `?` 0.5 / `n` 0.0 / 沈黙 0.25 |

これで H の意味が変わる。以前は否定が常に `+1.0` の定数だった。

```
confidence 0.9 のノードを否定  → E=0.90 → H_post +1.26   驚き
confidence 0.1 のノードを否定  → E=0.10 → H_post +0.14   想定内
confidence 0.05 のノードに同意 → E=0.95 → H_post +1.33   「通らないと思っていたのに通った」
```

3つ目が要点で、**否定だけが学習を駆動する非対称が解消される**。M_B の過小評価も
誤差なので再編を促す。予測が当たっている限り H はほとんど積まれない。

`/h` で E と次元ごとの予測信頼度を確認できる（信頼度は観測のみで制御には未使用）。
