...
追加した3件の回帰テストは修正前に2 failures / 1 errorとなり、既存341件は成功した。

- LLM生成APIの通信例外がCLIへ抜け、execution auditが残らない。
- 引用turnの一部が欠けた計画で、利用できる後続turnの評価器へ暗黙に切り替わる。
- executorが複数候補のplanでも、target-proposedのラベルと対象の包含だけでcallbackを呼べる。

修正は停止条件の強化に限定する。生成例外はgraph書込み前に無変更結果へ変換し、既存sessionの監査・明示再試行に接続する。graph書込み中の例外はこのcatchに含めず、部分変更を「安全に再試行できる無変更」と偽らない。plannerは引用turnの全件取得を要求し、executorは候補が対象一件だけであることを再検証する。

修正前テスト後にローカル環境の接続が失われたため、修正後の検証はGitHub Actionsで行う。通常入口切替、graph/audit二ファイルのクラッシュ原子性、実LLM品質は引き続き未完了。
